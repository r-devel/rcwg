...
* Have jpeg, libpng, pkgconfig, tiff and zlib-system-stub from r-base-dev 
* Got XQuartz 
* Got Obj C compiler with xcode command line tools 
* Got tcl/tk with standard installation of R binary
* Cairo support - easiest to install pango, will install cairo and
dependencies at same time
* I had MacTex already - assume most would??
* Got texinfo with standard installation of R binary
)
...
If you skipped the [Prerequisites](#Prerequisites) section because you believe 
you have the prerequisites already installed, you'll need to define the `LOCAL` 
environment variable to match your architecture
```{sh}
# Intel
export LOCAL=/opt/R/x86_64 
```
...
Run the following commands within Terminal.app to set up a local copy of the 
...
4. Configure the R installation with `--enable-R-framework` to prepare for installation as an App:
    ```{sh}
