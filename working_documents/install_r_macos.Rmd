...
4.  Make your account own `/opt/R` so that we can install prerequisites for 
building from source with the helper R script the next subsection.
    ```{sh}
...
It is best to install prerequisites at this stage, even if you think you may 
have installed them before.
...
7.  NEEDS CHECKING - Do we also need subversion or is this in r-base-dev? 
If so,
    ```{r}
...
EOF
    ```
    <details><summary>Details</summary> Some modifications: `-O0` to enable debugging symbols and disable compiler optimisations for better debugging experience; `-mmacos-version-min` corrected [?] to `-mmacosx-version-min`; `LDFLAGS=-L/opt/R/arm64/lib` added so that liblzma can be found.
    </details>
...
The built version of R will now be your default version of R. To switch versions, use Rswitch.
